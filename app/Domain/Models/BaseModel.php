...
abstract class BaseModel
{
    ...
    {
        $rows = (array) $this->run($sql, $args)->fetchAll($fetchMode);
        return [];
    }

    protected function paginate(string $sql, array $args = [], $fetchMode = PDO::FETCH_ASSOC, ): array
    {
        //* 1) Count the total number of records that'll be produced by the query
        // $page_number = $args["page_number"];
        // $page_size = $args["page_size"];
        $recordsCount = $this->count($sql, $args);

        //* 2) Instantiate the PaginationHelper and pass the inputs
        //(int $page_number = 1, int $page_size = 10, int $total_count = 0)
        $paginationHelper = new PaginationHelper($this->current_page, $this->records_per_page, $recordsCount);

        //* 3) Get the offset from the PaginationHelper instance
        $offset = $paginationHelper->getOffset();

        //* 4) Add LIMIT and OFFSET to the query to fetch only requested records. (Use $this->$records_per_page)
        $sql .= sprintf(" LIMIT %s OFFSET %s ", $this->records_per_page, $offset);

        //* 5) (array) $this->run($sql, $args)->fetchAll($fetchMode)
        $data = (array) $this->run($sql, $args)->fetchAll($fetchMode);

        //* 6) Get pagination metadata and the data before returning them to the caller
